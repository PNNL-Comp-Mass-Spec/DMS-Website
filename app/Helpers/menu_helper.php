...
function get_menu_context($page_type, $controller) {
    ...
    if (isset($controller->my_tag)) {
        switch ($page_type) {
            case 'List_Reports':
            case 'Param_Pages':
                $menu_context['clear_settings_link'] = "javascript:navBar.invoke(dmsNav.setListReportDefaults, \"$page_type\")";
                break;
        }
    ...
    $menu_context['side_panel_toggle'] = "<span style='margin:0;'><a title='Show/Hide side menu' href='javascript:dmsNav.toggle_frames();'><img src='" . base_url('/images/layout.png') . "' style='border-style:none'></a></span>";
