...
class Entry_form {
    ...
    function build_entry_array($mode): array {
        ...
        foreach ($this->form_field_specs as $fldName => $spec) {
            ...
            {
                $doc[] = "DO NOT EDIT THIS FIELD - autogenerated and used as identity, or auto-filled";
                $nonEdit = true;
            }
            if (in_array('text-if-new', $fieldTypes))
            {
                $doc[] = "WRITE-ONCE FIELD - do not edit when updating";
            }
            if (in_array('hidden', $fieldTypes))
            {
                $doc[] = "hidden field, do not change this value";
            }
            ...
            if (!$nonEdit)
            {
                $chooserText = $this->controller->getChoosers()->make_choosers_doc($fldName, $spec);
                if (strlen($chooserText) > 0)
                {
                    $doc[] = $chooserText;
                }
                else
                {
                    if (in_array('alpha_dash', $fieldRules))
                    {
                        $doc[] = "must match regex [A-Za-z0-9_\-]+";
                    }
                    if (in_array('alpha_numeric', $fieldRules))
                    {
                        $doc[] = "must match regex [A-Za-z0-9]+";
                    }
                    if (in_array('numeric', $fieldRules))
                    {
                        $doc[] = "must match regex [0-9]+";
                    }
                    ...
                    foreach ($fieldRules as $rule)
                    {
                        ...
                        $ruleParam = trim($split[1], '[] ');
                        if ($split[0] == 'not_contain')
                        {
                            $doc[] = "must not contain [" . $ruleParam . "]";
                        }
                    ...
                    if (count($lengthRules) > 0)
                    {
                        $doc[] = "length must be " . implode(', ', $lengthRules);
                    }
                }
            }
